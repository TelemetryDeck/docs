...
tags:
  ...
  - Docs
  - Events
description: A reference of all properties of a Event
lead: A Event is the representation of one event happening in one instance of your app. Here is an overview of its properties.
---

Events are an indication that **an event** happened in your app, which is used by a **user**. Events consist of these parts:

- **Event Type** – A string that indicates which kind of event happened.<br>In this case we'll use `applicationDidFinishLaunching`, but it can be `databaseUpdated` or `settingsScreenOpened` or `pizzaModeActivated` (I totally love that last one!)
- **User Identifier** – A string that identifies your user.<br>This can be an email address or a username, or a random ID that you generate once and store somewhere. It should always be the same for all the events you send from a certain instance of the app. If you don't supply a user identifier, `TelemetryDeck` will generate one for you.
...
As TelemetryDeck is an analytics software, it analyzes events that occur in your apps' life cycles. Through aggregation and statistical analysis, your
[Insights](/docs/api/insights-reference/) can then extract meaningful data out of the set of events you have received.
...
Events always have a type. This is a short string that describes the event just happened. It is
...
When you're setting up your [Insights](/docs/api/insights-reference/) later, you'll be able to **filter** by Event Type.

## Client User

Events have a `clientUser` property, which stores a string. All events with the same client user property will be
...
an empty string instead. This will effectively disable user counting, but you can still count sessions and events,
...
Events have a metadata payload dictionary that contains things like platform, os version, and any data you throw in
...
Events have a "receivedAt at" property that is set to the time (in UTC) when the event was generated on the device. This
allows you to group them by time.

<div class="alert alert-info" role="alert">
Note: In a future version of TelemetryDeck, you'll be able to <a href="https://github.com/TelemetryDeck/SwiftSDK/issues/19">store events locally in the app before sending them</a>.
If you are using that technique, the server will no longer set the time the event was created, but it will instead be
set on the client.
</div>
